...
foreach ($metaFile in $metas) {
    ...
    try {
        ...
        $isRaw = ($meta["kind"] -eq "raw")
        if ($isRaw) {
            # 게임 본체 데이터 파일(level0, sharedassets0.assets): MindsBeneathUs_Data 폴더에 있음. 번들이 아니라 파일 자체에 diff 적용.
            $dataDir = Join-Path $gameRoot "MindsBeneathUs_Data"
            $targetPath = Join-Path $dataDir $bundleName
            $rawBackupDir = Join-Path $dataDir "_originals_backup_player"
            if (-not (Test-Path $rawBackupDir)) { New-Item -ItemType Directory -Path $rawBackupDir | Out-Null }
            $backupPath = Join-Path $rawBackupDir $bundleName
        } else {
        ...
        if ($isRaw) {
            Log "  diff 적용 중..."
            $outTmp = Join-Path $tmpDir "raw.out"
            & $xdelta -d -f -s $srcPath $xdPath $outTmp
            if ($LASTEXITCODE -ne 0) { throw "xdelta3 실패 (exit $LASTEXITCODE)" }
            if ((Get-Sha256 $outTmp) -ne $meta["new_sha256"]) { throw "diff 적용 결과 검증 실패 (해시 불일치)" }
            if (-not (Test-Path $backupPath)) { Copy-Item -Path $targetPath -Destination $backupPath }
